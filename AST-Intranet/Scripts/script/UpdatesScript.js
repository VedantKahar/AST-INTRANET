const toggleSidebar = document.getElementById('toggleSidebar');
        ...
        // Handle sidebar hover behavior (only works when not clicked)
        ...
        const today = new Date();
        const start = new Date(today.getFullYear(), 0, 0); // January 1st
        const diff = today - start; // Difference in milliseconds
        const oneDay = 1000 * 60 * 60 * 24; // Milliseconds in one day
        const dayOfYear = Math.floor(diff / oneDay); // Get the day of the year (1-365)

        // Select a quote based on the day of the year
        const quoteOfTheDay = quotes[dayOfYear % quotes.length]; // Use modulo to loop through the quotes
